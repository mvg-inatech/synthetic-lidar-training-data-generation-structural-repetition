...
class CheckpointLoader(HookBase):
    def __init__(self, keywords="", replacement=None, strict=False, reinit_ptv3_stem_channels=None, reinit_ptv3_stem_channels_map=None):
        self.keywords = keywords
        self.replacement = replacement if replacement is not None else keywords
        self.strict = strict
        self.reinit_ptv3_stem_channels = reinit_ptv3_stem_channels
        self.reinit_ptv3_stem_channels_map = reinit_ptv3_stem_channels_map
    ...
    def before_train(self):
        ...
        if self.trainer.cfg.weight and os.path.isfile(self.trainer.cfg.weight):
            ...
            # Support both list and string types for keywords and replacement
            # If both are lists, apply pairwise replacements (new configs); otherwise use string logic (original pointcept variant)
            is_list_mode = isinstance(self.keywords, list) and isinstance(self.replacement, list)
            if is_list_mode:
                assert len(self.keywords) == len(self.replacement), (
                    f"keywords and replacement must have the same length: "
                    f"keywords={len(self.keywords)}, replacement={len(self.replacement)}"
                )
                self.trainer.logger.info(
                    f"Loading layer weights with keywords: {self.keywords}, "
                    f"replacements: {self.replacement}"
                )
            else:
                self.trainer.logger.info(
            ...
            for key, value in checkpoint["state_dict"].items():
                ...
                if is_list_mode:
                    # Apply each keyword-replacement pair in order
                    for keyword, replacement in zip(self.keywords, self.replacement):
                        if keyword in key:
                            key = key.replace(keyword, replacement, 1)
                else:
                ...
                # Handle channel mapping for PTv3 stem weight
                if self.reinit_ptv3_stem_channels_map is not None and "embedding.stem.linear.weight" in key:
                    value = self._convert_ptv3_stem_channels(key, value)
            ...
            if self.reinit_ptv3_stem_channels is not None:
                self._reinit_ptv3_stem_channels()
    ...
    def _reinit_ptv3_stem_channels(self):
        from timm.models.layers import trunc_normal_

        # Logic is from class PointTransformerV3 -> _init_weights()

        model = self.trainer.model
        if hasattr(model, "module"):
            model = model.module

        linear = model.backbone.embedding.stem.linear
        with torch.no_grad():
            trunc_normal_(linear.weight[:, self.reinit_ptv3_stem_channels], std=0.02)
        self.trainer.logger.info(
            f"Re-initialized PTv3 embedding input channels {self.reinit_ptv3_stem_channels} from scratch."
        )

    def _convert_ptv3_stem_channels(self, key, value):
        """
        Convert PTv3 stem weight from pretrained channels to target channels.

        Args:
            key: The weight key (e.g., "embedding.stem.linear.weight")
            value: The weight tensor from pretrained checkpoint

        Returns:
            The converted weight tensor with shape (target_channels, embed_channels)
        """
        # This is the stem linear layer weight
        # value shape: (source_channels, embed_channels), need to convert to (target_channels, embed_channels)
        self.trainer.logger.info(f"Processing stem weight: {key}, shape: {value.shape}")

        # Get the channel mapping
        target_channels = list(self.reinit_ptv3_stem_channels_map.keys())
        source_channels = [self.reinit_ptv3_stem_channels_map[t] for t in target_channels]

        # Select only the source channels from pretrained weight
        selected_weight = value[:, source_channels]  # (embed_channels, source_channels)

        # Create a new weight tensor with all target channels
        # Channels in the map get pretrained weights, others get zero
        embed_channels = value.shape[0]
        new_weight = torch.zeros((embed_channels, max(target_channels) + 1), dtype=value.dtype, device=value.device)

        # Copy selected weights to their target positions
        for i, target_idx in enumerate(target_channels):
            new_weight[:, target_idx] = selected_weight[:, i]

        self.trainer.logger.info(f"Converted stem weight from {value.shape} to {new_weight.shape}")
        return new_weight
