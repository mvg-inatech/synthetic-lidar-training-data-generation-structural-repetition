...
class SemSegTester(TesterBase):
    def test(self):
        ...
        if (
            self.cfg.data.test.type == "ScanNetDataset" or self.cfg.data.test.type == "ScanNet200Dataset" or self.cfg.data.test.type == "ScanNetPPDataset"
        ...
        elif (self.cfg.data.test.type == "SubwayDataset" or self.cfg.data.test.type == "STSDDataset") and comm.is_main_process():
            test_preds_save_path = os.path.join(save_path, "test_preds", time.strftime("%Y-%m-%d_%H-%M-%S"))
            make_dirs(test_preds_save_path)
        ...
        for batch_idx, data_dict in enumerate(self.test_loader):
            ...
            if os.path.isfile(pred_save_path):
                logger.info("{}/{}: {}, loaded pred and label.".format(batch_idx + 1, len(self.test_loader), data_name))
                pred = np.load(pred_save_path)
                if "origin_segment" in data_dict.keys():
                    segment = data_dict["origin_segment"]
            else:
                pred = torch.zeros((segment.size, self.cfg.data.num_classes)).cuda()
                for fragment_idx in range(len(fragment_list)):
                    fragment_batch_size = 1
                    s_i, e_i = fragment_idx * fragment_batch_size, min((fragment_idx + 1) * fragment_batch_size, len(fragment_list))
                    ...
                    logger.info(
                        "Test: {}/{} {data_name}, fragment {fragment_idx}/{fragment_num}".format(
                            batch_idx + 1,
                ...
                elif self.cfg.data.test.type == "SubwayDataset" or self.cfg.data.test.type == "STSDDataset":
                    # Normalize accumulated probabilities to get confidence values between 0 and 1
                    pred_normalized = pred / pred.sum(dim=1, keepdim=True)
                    pred_confidence = pred_normalized.max(1)[0].data.cpu().numpy()
                ...
                if "origin_segment" in data_dict.keys():
                    ...
                    if self.cfg.data.test.type == "SubwayDataset" or self.cfg.data.test.type == "STSDDataset":
                        pred_confidence = pred_confidence[data_dict["inverse"]]
                    segment = data_dict["origin_segment"]

                if self.cfg.data.test.type != "SubwayDataset":
            ...
            if self.cfg.data.test.type == "ScanNetDataset" or self.cfg.data.test.type == "ScanNet200Dataset":
                ...
                NUM_SCENES_TO_EXPORT = 25
                if (NUM_SCENES_TO_EXPORT == -1 or batch_idx < NUM_SCENES_TO_EXPORT) and comm.is_main_process():
                    import laspy

                    # Pointcept does the testing in many fragments per scene. Reconstruct the full scene for .laz export.
                    total_num_points = int(segment.size)
                    scene_coords = np.empty((total_num_points, 3), dtype=np.float64)
                    for frag in fragment_list:
                        if "origin_coord" in frag:
                            frag_coords = frag["origin_coord"]
                        else:
                            frag_coords = frag["coord"]

                        fragment_points_coords = frag_coords.detach().cpu().numpy()
                        fragment_points_idx = frag["index"].detach().cpu().numpy()
                        scene_coords[fragment_points_idx] = fragment_points_coords

                    export_las_path = os.path.join(test_preds_save_path, f"{data_name}.laz")
                    las_header = laspy.header.LasHeader(point_format=6)
                    las_header.add_extra_dim(laspy.ExtraBytesParams(name="label_gt", type=np.int8))
                    las_header.add_extra_dim(laspy.ExtraBytesParams(name="label_pred", type=np.int8))
                    las_header.add_extra_dim(laspy.ExtraBytesParams(name="pred_confidence", type=np.float32))
                    export_las = laspy.LasData(las_header)
                    export_las.x, export_las.y, export_las.z = (
                        scene_coords[:, 0],
                        scene_coords[:, 1],
                        scene_coords[:, 2],
                    )
                    export_las.label_gt = segment
                    export_las.label_pred = pred
                    export_las.pred_confidence = pred_confidence
                    export_las.write(export_las_path)
            ...
            mask = union != 0  # mask to only include classes that appear in the scene
            ...
            # Track individual scene results
            record[data_name]["miou"] = iou  # Classes that are not present in the scene are excluded from the mean calculation (by `mask` above)
            record[data_name]["iou_class"] = iou_class

            batch_time.update(time.time() - start)
            logger.info(
                "Test: {} [{}/{}]-{} Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) Accuracy {acc:.4f} ({m_acc:.4f}) mIoU {iou:.4f} ({m_iou:.4f})".format(
                    data_name,
                    batch_idx + 1,
                    len(self.test_loader),
                    segment.size,
        ...
        if comm.is_main_process():
            ...
            # Rank scenes by mIoU for printing best and worst
            NUM_BEST_WORST_TO_SHOW = 25
            logger.info("\n" + "=" * 40)
            logger.info("Best and worst scenes")
            logger.info("=" * 40)

            scene_results = [(name, meters["miou"], meters["iou_class"]) for name, meters in record.items()]
            scene_results_sorted = sorted(scene_results, key=lambda x: x[1], reverse=True)  # Sort by mIoU

            best_scenes = scene_results_sorted[:NUM_BEST_WORST_TO_SHOW]
            worst_scenes = scene_results_sorted[-NUM_BEST_WORST_TO_SHOW:][::-1]  # Reverse to show worst first

            # Print best scenes
            logger.info("\n" + "-" * 40)
            logger.info(f"Top {NUM_BEST_WORST_TO_SHOW} best scenes:")
            logger.info("-" * 40)
            for rank, (scene_name, scene_miou, scene_iou_class) in enumerate(best_scenes, 1):
                logger.info(f"Rank {rank}: {scene_name}")
                logger.info(f"  mIoU: {scene_miou:.4f}")
                logger.info("  Per-class IoU:")
                for i in range(self.cfg.data.num_classes):
                    if scene_iou_class[i] > 0 or i < len(self.cfg.data.names):
                        logger.info(f"    {self.cfg.data.names[i]}: {scene_iou_class[i]:.4f}")
                logger.info("")

            # Print worst scenes
            logger.info("\n" + "-" * 40)
            logger.info(f"Top {NUM_BEST_WORST_TO_SHOW} worst scenes:")
            logger.info("-" * 40)
            for rank, (scene_name, scene_miou, scene_iou_class) in enumerate(worst_scenes, 1):
                logger.info(f"Rank {rank}: {scene_name}")
                logger.info(f"  mIoU: {scene_miou:.4f}")
                logger.info("  Per-class IoU:")
                for i in range(self.cfg.data.num_classes):
...
class ClsTester(TesterBase):
    def test(self):
        ...
        for i, input_dict in enumerate(self.test_loader):
            ...
            logger.info(
                "Test: [{}/{}] Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) Accuracy {accuracy:.4f} ".format(
                    i + 1,
                    len(self.test_loader),
                    batch_time=batch_time,
                    accuracy=accuracy,
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            for key in input_dict.keys():
                if isinstance(input_dict[key], torch.Tensor):
                    input_dict[key] = input_dict[key].cuda(non_blocking=True)
            with torch.no_grad():
                pred = F.softmax(self.model(input_dict)["cls_logits"], -1).sum(0, keepdim=True)
            pred = pred.max(1)[1].cpu().numpy()
            intersection, union, target = intersection_and_union(pred, category, self.cfg.data.num_classes, self.cfg.data.ignore_index)
            intersection_meter.update(intersection)
            target_meter.update(target)
            ...
            batch_time.update(time.time() - end)
            logger.info(
                "Test: {} [{}/{}] Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) Accuracy {acc:.4f} ({m_acc:.4f}) ".format(
                    data_name,
                    idx + 1,
                    len(self.test_loader),
                    batch_time=batch_time,
...
class PartSegTester(TesterBase):
    def test(self):
        ...
        for idx in range(len(test_dataset)):
            ...
            batch_time.update(time.time() - end)
            logger.info(
                "Test: {} [{}/{}] Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) ".format(data_name, idx + 1, len(self.test_loader), batch_time=batch_time)
            )
...
class InsSegTester(TesterBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.valid_class_names = [self.cfg.data.names[i] for i in range(self.cfg.data.num_classes) if i not in self.segment_ignore_index]
    ...
    def test(self):
        ...
        logger.info("Val result: mAP/AP50/AP25 {:.4f}/{:.4f}/{:.4f}.".format(all_ap, all_ap_50, all_ap_25))
        for i, label_name in enumerate(self.valid_class_names):
            ap = ap_scores["classes"][label_name]["ap"]
            ap_50 = ap_scores["classes"][label_name]["ap50%"]
            ap_25 = ap_scores["classes"][label_name]["ap25%"]
            logger.info("Class_{idx}-{name} Result: AP/AP50/AP25 {AP:.4f}/{AP50:.4f}/{AP25:.4f}".format(idx=i, name=label_name, AP=ap, AP50=ap_50, AP25=ap_25))
        logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
