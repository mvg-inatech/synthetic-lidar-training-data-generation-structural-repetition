...
class SubwayDataset(DefaultDataset):
    """
    Synth Subway dataset for semantic and instance segmentation.

    The dataset should be preprocessed using preprocess_synth_subway.py
    and organized in the following structure:

    subway/
    ├── synth_train/
    │   ├── synth_seed-1_part-001_OusterOS0LowRes.las
    │   ├── synth_seed-1_part-002_OusterOS0LowRes.las
    │   └── ...
    ├── synth_val/
    │   └── ...
    ├── synth_test/
    ...
    ├── real_test/
    │   └── ...
    ├── real_dataset_info.json
    └── synth_dataset_info.json
    """
    ...
    def get_data(self, idx):
        ...
        assert not np.any(segment > 7), f"Warning: Found labels > 7 in {self.get_data_name(idx)}, max label: {segment.max()}"
