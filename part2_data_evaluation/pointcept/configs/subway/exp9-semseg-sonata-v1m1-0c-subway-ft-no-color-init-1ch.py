    ...
    train=dict(
        ...
        transform=[
            ...
            dict(
                ...
                feat_keys=("coord", "strength", "normal"),
            ),
        ],
        test_mode=False,
    ),
...
hooks = [
    dict(
        ...
        reinit_ptv3_stem_channels_map={  # Map channels from 9ch pretrained weights to 7ch model
            # Target channel 0, 1, 2 (coord) -> Source channel 0, 1, 2 (coord)
            0: 0,
            1: 1,
            2: 2,
            # Target channel 3 (intensity) -> No source (will be re-initialized from scratch)
            # Target channel 4, 5, 6 (normal) -> Source channel 6, 7, 8 (normal)
            4: 6,
            5: 7,
            6: 8,
        },
        reinit_ptv3_stem_channels=[3],  # Initialize target channel 3 (intensity) from scratch
    ),
