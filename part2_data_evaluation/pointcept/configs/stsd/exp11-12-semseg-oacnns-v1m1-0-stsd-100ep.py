# Config is taken from pointcept repo, scannet semseg-oacnns-v1m1-0-base, modified

_base_ = ["../_base_/default_runtime.py"]

dataset_type = "STSDDataset"
data_root = "data/stsd"
...
ignore_index = -100  # This value is not used in the STSD dataset, but the variable must be set to some value.
...
epoch = 100
eval_epoch = 100  # Make this equal to `epoch`, otherwise the dataset will be looped (see defaults.py)
...
hooks = [
    dict(
        # This hook is only applied during training (see test.py build_model() code)
        type="CheckpointLoader",
        # Since the number of classes in loaded checkpoints (for synthetic pretraining) differs from the number of classes in this STSD dataset, skip loading weights for the final classification layer and initialize them from scratch.
        keywords="backbone.final",
        replacement="backbone.final_IGNORE_DO_NOT_LOAD_FROM_CHECKPOINT",
    ),
